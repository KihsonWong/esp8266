...
#endif
LOCAL os_timer_t rainbow_heart_timer;
...
extern bool ICACHE_FLASH_ATTR
checkup_data(char *precv, uint16 length);
/******************************************************************************
...
user_esp_platform_recv_cb(void *arg, char *pusrdata, unsigned short length)
{
	...
	re_value = checkup_data(pusrdata, length);
	if (re_value)
...
user_esp_platform_recon_cb(void *arg, sint8 err)
{
    ...
    ESP_DBG("rainbow reconnect cloud server cb\n");
...
struct espconn *esp_user_uart_client;//send data to server
/******************************************************************************
...
*******************************************************************************/
void ICACHE_FLASH_ATTR
user_esp_hold_heart(uint8 flag)
{
	char heart[24] = "HOLD ON CONNECTING HEART" ;

	espconn_send(esp_user_uart_client, heart, 24);
}
/******************************************************************************
...
user_esp_platform_connect_cb(void *arg)
{
    ...
    ESP_DBG("rainbow connected cloud server cb\n");
    if (wifi_get_opmode() ==  STATIONAP_MODE ) {
        wifi_set_opmode(STATION_MODE);
    }
    os_timer_disarm(&rainbow_heart_timer);
    os_timer_setfn(&rainbow_heart_timer, (os_timer_func_t *)user_esp_hold_heart, 0);
    os_timer_arm(&rainbow_heart_timer, 20000, 1);
#if (PLUG_DEVICE || SENSOR_DEVICE)
...
{
    ESP_DBG("rainbow is connecting cloud server\n");
...
extern struct espconn *esp_user_uart;
void return_dhcp_ip_address()
{
	char IpBuffer[40] = {0};
	struct ip_info sta_info;
	wifi_get_ip_info(STATION_IF,&sta_info);
	os_sprintf(IpBuffer, IPSTR, IP2STR(&sta_info.ip));
	espconn_send(esp_user_uart, IpBuffer, 40);
}
...
user_esp_platform_check_ip(uint8 reset_flag)
{
    ...
    if (wifi_station_get_connect_status() == STATION_GOT_IP && ipconfig.ip.addr != 0) {
...
#else
        const char esp_server_ip[4] = {176, 122, 178, 169};
...
#else
        user_conn.proto.tcp->remote_port = 8181;
#endif
        ...
        esp_user_uart_client = &user_conn;
#endif
    } else {
        ...
        if ((wifi_station_get_connect_status() == STATION_WRONG_PASSWORD ||
        	...
        	wifi_set_opmode(STATIONAP_MODE);
        	//os_printf("Connection to router failed! change ap...\r\n");
            //user_esp_platform_reset_mode();
        } else {
