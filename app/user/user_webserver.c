...
checkup_data(char *precv, uint16 length)
{
    ...
    pssid = (char *)os_strstr(precv, "ssid: ");
    ppassword = (char *)os_strstr(precv, "password: ");
    end = (char *)os_strstr(precv, "end");

    ask = (char *)os_strstr(precv, "Who are you?");

    if ((pssid != NULL) && (ppassword != NULL) && (end != NULL)) {
    	ssid_len = ppassword - pssid;
    	ssid_len -= 7;
    	password_len = end - ppassword;
    	password_len -= 11;
    	wifi_station_get_config(&config); // Get config first.
    	wifi_station_disconnect();

    	os_memset(config.ssid, 0, 32);
    	os_memset(config.password, 0, 64);
    	os_memcpy(config.ssid, pssid + 6, ssid_len);
    	os_memcpy(config.password, ppassword + 10, password_len);

    	config.bssid_set = 0;
		wifi_station_set_config(&config);// Set ESP8266 station config .
		wifi_station_connect();

		// Set timer to check whether router allotted an IP
		os_timer_disarm(&test_timer);
		os_timer_setfn(&test_timer, (os_timer_func_t *)user_esp_platform_check_ip, 0);
		os_timer_arm(&test_timer, 1000, 0);
		return false;
    } else if (ask != NULL) {
    	extern struct espconn *esp_user_uart_client;
    	espconn_send(esp_user_uart_client, "I'm rainbow 10", 14);
    	return false;
    } else {
    	return true;
    }
 ...
 * Returns      : none
*******************************************************************************/
struct espconn *esp_user_uart = NULL;
void ICACHE_FLASH_ATTR
user_webserver_init(uint32 port)
{
    ...
    espconn_regist_time(&esp_conn, 3600, 0);
    esp_user_uart = &esp_conn;
#endif
}
