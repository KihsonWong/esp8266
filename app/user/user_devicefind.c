...
#define UART0   0
void uart0_send_buf(char *p, u16 len)
{
	while(len--) uart_tx_one_char(UART0, *p++);
}
/*---------------------------------------------------------------------------*/
...
user_devicefind_recv(void *arg, char *pusrdata, unsigned short length)
{
    ...
    if (1//length == os_strlen(device_find_request) /*&&
...
//
//        os_printf("%s\n", DeviceBuffer);
    	uart0_send_buf(pusrdata, length);
...
//        length = os_strlen(DeviceBuffer);
        ...
        //espconn_sent(&ptrespconn, pusrdata, length);
...
user_devicefind_init(void)
{
    ...
    ptrespconn.proto.udp->local_port = 1012;
