...
LOCAL char rcv_buf[1024];
LOCAL int rcv_num = 0;
LOCAL os_timer_t rcv_timer;
extern struct espconn *esp_user_uart;
extern struct espconn *esp_user_uart_client;
void send_wifi_data(char *rcv)
{
	espconn_send(esp_user_uart, rcv, rcv_num);
	espconn_send(esp_user_uart_client, rcv, rcv_num);
	rcv_num = 0;
}
LOCAL void ICACHE_FLASH_ATTR ///////
uart_recvTask(os_event_t *events)
{
    if(events->sig == 0){
        ...
        uint8 idx=0;
/*        if(rcv_num == 0) {
        	rcv_time = system_get_time() + 10000;
        }*/
        for(idx=0;idx<fifo_len;idx++) {
            d_tmp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
            rcv_buf[rcv_num++] = d_tmp;
            if (d_tmp == "\n") send_wifi_data(rcv_buf);
            //uart .0_tx_one_char(UART0, d_tmp);
        }
//        os_timer_disarm(&rcv_timer);
//        os_timer_setfn(&rcv_timer, (os_timer_func_t *)send_wifi_data, rcv_buf);
//        os_timer_arm(&rcv_timer, 100, 0);
